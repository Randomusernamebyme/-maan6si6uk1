...
export async function uploadGalleryBufferToFolder(
  buffer: Buffer,
  contentType: string,
  folderSuffix: string
): Promise<{ secureUrl: string; publicId: string }> {
  ...
  if (!safeSuffix) {
    throw new Error("缺少 Cloudinary folder");
  }
  const folder = `${prefix}/${safeSuffix}`;
  const timestamp = Math.round(Date.now() / 1000);
...
export async function uploadImageBufferToPublicId(
  buffer: Buffer,
  contentType: string,
  publicId: string
): Promise<{ secureUrl: string; publicId: string }> {
  ...
  const safePublicId = String(publicId || "").trim().replace(/^\/+|\/+$/g, "");
  if (!safePublicId) {
    throw new Error("缺少 Cloudinary public_id");
  }
  ...
  return { secureUrl: data.secure_url, publicId: data.public_id || safePublicId };
}
